...
- `report/report.tex` → `report/report.pdf` — 11-page technical report with a self-contained
  appendix (API reference, data model, config, algorithm pseudocode, build/run). Build with
  `tectonic report.tex` or `pdflatex report.tex` twice.
